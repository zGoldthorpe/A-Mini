...
class Syntax:
    ...
    def check(self, func_name, *args, **kwargs):
        ...
        for i, (arg, ty) in enumerate(zip(args, self._types)):
            if not isinstance(arg, ty):
                raise TypeError(f"{func_name} argument {i} expects {ty.__name__}; {type(arg).__name__} given.")
        for kw in kwargs:
            ...
            if not isinstance(kwargs[kw], self._kwtypes[kw]):
                raise TypeError(f"{func_name} keyword argument {kw} expects {self._kwtypes[kw].__name__}; {type(kwargs[kw]).__name__} given.")
    ...
    def __repr__(self):
        ...
        keyword = ", ".join(f"{kw}:{self._kwtypes[kw].__name__}" for kw in self._kwtypes)
...
def ensure_multi(*syntaxes):
    ...
    def outer(func):
        ...
        def inner(*args, **kwargs):
            ...
            raise TypeError(f"""{func.__name__} received unrecognised argument pattern
